...
namespace PianoForte.Manager
{
    public class ReportManager
    {
        public static DataSet initReceiptReportTable(int paymentId)
        {
            ...
            DataTable dataTablePayment = ReportManager.initDataTablePayment();
            DataTable dataTablePaymentDetail = ReportManager.initDataTablePaymentDetail();
            DataTable dataTableReceiptFooter = ReportManager.initDataTableReceiptFooter();
            DataTable dataTableStudent = ReportManager.initDataTableStudent();
            DataTable dataTableUser = ReportManager.initDataTableUser();

            Payment payment = PaymentManager.findPayment(paymentId);
            if (payment != null)
            {
                dataTablePayment = ReportManager.addDataToDataTablePayment(dataTablePayment, payment);
                dataTableStudent = ReportManager.addDataToDataTableStudent(dataTableStudent, payment.StudentId);
                dataTablePaymentDetail = ReportManager.addDataToDataTablePaymentDetail(dataTablePaymentDetail, paymentId);
                dataTableReceiptFooter = ReportManager.addDataToDataTableReceiptFooter(dataTableReceiptFooter, paymentId);
                dataTableUser = ReportManager.addDataToDataTableUser(dataTableUser, payment.ReceiverId);
        ...
        public static DataSet initReceiptReportTable(Payment payment)
        {
            ...
            if (payment != null)
            {
                dataTablePayment = ReportManager.addDataToDataTablePayment(dataTablePayment, payment);
                dataTableStudent = ReportManager.addDataToDataTableStudent(dataTableStudent, payment.StudentId);
                dataTablePaymentDetail = ReportManager.addDataToDataTablePaymentDetail(dataTablePaymentDetail, payment.Id);
                dataTableReceiptFooter = ReportManager.addDataToDataTableReceiptFooter(dataTableReceiptFooter, payment.Id);
                dataTableUser = ReportManager.addDataToDataTableUser(dataTableUser, payment.ReceiverId);
        ...
        {
            List<Enrollment> enrollmentList = EnrollmentManager.findAllEnrollmentByTransactionId(paymentId);
        ...
        public static bool printReceipt(Payment payment)
        {
            ...
            try
            {
                DataSet dataSet = ReportManager.initReceiptReportTable(payment);
